...
class CBORReader(private val bytes: ByteArray) {
    ...
    private fun nextByte(): Byte? {
        return if (this.cursor < this.size) {
            ...
            Fido2Logger.debug(TAG, "no enough size")
            null
        }
    }
    ...
    fun readFloat(): Float? {
        ...
        if (b1 != CBORBits.floatBits) {
            Fido2Logger.debug(TAG, "Invalid 'float' format")
            return null
        }
    ...
    fun readDouble(): Double? {
        ...
        if (b1 != CBORBits.doubleBits) {
            Fido2Logger.debug(TAG, "Invalid 'double' format")
            return null
        }
    ...
    fun readByteString(): ByteArray? {
        ...
        if ((b1 and CBORBits.headerPart) != CBORBits.bytesHeader) {
            Fido2Logger.debug(TAG, "Invalid 'bytes' format")
            return null
        }
    ...
    fun readString(): String? {
        ...
        if ((b1 and CBORBits.headerPart) != CBORBits.stringHeader) {
            Fido2Logger.debug(TAG, "Invalid 'string' format")
            return null
        }
    ...
    fun readAny(): Any? {
        ...
        when {
            ...
            (v1 and CBORBits.headerPart) == CBORBits.mapHeader -> {
                ...
                Fido2Logger.debug(TAG, "Unsupported value type for 'Any'")
                return null
            }
        }
    ...
    fun readIntKeyMap(): Map<Int, Any>? {
        ...
        if ((b1 and CBORBits.headerPart) != CBORBits.mapHeader) {
            Fido2Logger.debug(TAG, "Invalid 'map' format")
            return null
        }
    ...
    fun readArray(): List<Any>? {
        ...
        if ((b1 and CBORBits.headerPart) != CBORBits.arrayHeader) {
            Fido2Logger.debug(TAG, "Invalid 'array' format")
            return null
        }
    ...
    fun readNumber(): Long? {
        ...
        val bytesToRead = when (value) {
            ...
            27 -> 8
                ...
                Fido2Logger.debug(TAG, "Invalid 'number' format")
                return null
            }
